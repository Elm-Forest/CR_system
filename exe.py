...
from models.UnCRtainTS.test import pred_uncrtain, get_uncrtain
...
class Cloud_Removal:
    def __init__(self):
        ...
        self.criterion_vgg = lpips.LPIPS(net='alex').to(self.device)
        self.meta_learner = load_model(META_WEIGHTS, device=self.device)
        self.meta_learner.eval()
        self.CR_net = load_glf(device=self.device)
        self.dsencr = load_dsen()
        self.uncrtain = get_uncrtain()

    def predict_meta(self, image_name='default'):
        device = self.device
        ...
        sar = images['sar']
        with torch.no_grad():
            out = self.meta_learner(inputs.to(device).unsqueeze(dim=0), inputs2.to(device).unsqueeze(dim=0),
        ...
        criterion_vgg = self.criterion_vgg
        ssim_value = ssim(torch.from_numpy(outputs_rgb).unsqueeze(0), targets[1:4, :, :].unsqueeze(0),
                          window_size=5).item()
    ...
    def predict_glf(self, image_name='default'):
        device = self.device
        target_path = os.path.join(SOURCE_DIR, 'target', image_name + '.tif')
        cloudy_path = os.path.join(SOURCE_DIR, 's2', image_name + '.tif')
        sar_path = os.path.join(SOURCE_DIR, 's1', image_name + '.tif')
        ...
        with torch.no_grad():
            outputs = CR_net(cloudy.unsqueeze(0).to(device), sar.unsqueeze(0).to(device))
    ...
    def predict_dsen(self, image_name='default'):
        ...
        with torch.no_grad():
            predicted = self.dsencr.predict_on_batch(data)
    ...
    def predict_uncrtain(self, image_name='default'):
        target_path = os.path.join(SOURCE_DIR, 'target', image_name + '.tif')
        cloudy_path = os.path.join(SOURCE_DIR, 's2', image_name + '.tif')
        sar_path = os.path.join(SOURCE_DIR, 's1', image_name + '.tif')
        images = build_data(None, target_path, cloudy_path, sar_path, None, PREPROCESS)
    ...
    def postprocess_images(self, image_name):
        ...
        loss1_L1 = F.l1_loss(inputs[1:4, :, :].unsqueeze(0).to(device),
                             targets[1:4, :, :].to(device).unsqueeze(0)).item()
        lpips1 = criterion_vgg(inputs[1:4, :, :].unsqueeze(0).to(device),
                               targets[1:4, :, :].to(device).unsqueeze(0)).item()
        ...
        loss2_L1 = F.l1_loss(inputs2[1:4, :, :].unsqueeze(0).to(device),
                             targets[1:4, :, :].to(device).unsqueeze(0)).item()
        ...
        loss3_L1 = F.l1_loss(inputs3[1:4, :, :].unsqueeze(0).to(device),
                             targets[1:4, :, :].to(device).unsqueeze(0)).item()
...
def load_glf(device):
    checkpoint = torch.load(GLF_WEIGHTS)
    skip_keys = [
...
if __name__ == '__main__':
    ...
    cr = Cloud_Removal()
    out1 = cr.preprocess_images(file_name)
    out2 = cr.predict_dsen(file_name)
    out3 = cr.predict_glf(file_name)
    out4 = cr.predict_uncrtain(file_name)
    out5 = cr.postprocess_images(file_name)
    out6 = cr.predict_meta(file_name)
    print(out6)
