...
from models.UnCRtainTS.src.backbones import base_model, utae, uncrtaints
...
def load_model(config, model, train_out_layer=True, load_out_partly=True):
    ...
    try:  # try loading checkpoint strictly, all weights must match
        ...
        if load_out_partly:
            ...
            temp_weights, temp_biases = model_dict['out_conv.conv.conv.0.weight'], model_dict[
                'out_conv.conv.conv.0.bias']
...
def load_checkpoint(config, checkp_dir, model, name):
    ...
    try:  # try loading checkpoint strictly, all weights & their names must match
        model.load_state_dict(checkpoint, strict=True, map_location=torch.device('cpu'))
    except:
        # rename keys
