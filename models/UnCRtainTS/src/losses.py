...
def get_loss(config):
    if config.loss == "GNLL":
        ...
        criterion1 = MultiGaussianNLLLoss(reduction='mean', eps=1e-8, full=True, mode=config.covmode,
...
def multi_diag_gaussian_nll(pred, target, var):
    ...
    xTCx = torch.bmm(torch.bmm(err, prec), err.permute(0, 2, 1)).squeeze().nan_to_num().clamp(
        min=1e-9)  # note: equals torch.bmm(torch.bmm(-err, prec), -err)
    # define the NLL loss
...
def multi_gaussian_nll_loss(
    ...
    if mode in ['iso', 'diag']:
        mapdims = (-1, -1, -1)
        loss, variance = vmap(vmap(multi_diag_gaussian_nll, in_dims=mapdims, chunk_size=chunk), in_dims=mapdims,
                              chunk_size=chunk)(input, target, var)
...
class MultiGaussianNLLLoss(_Loss):
    ...
    def __init__(self, *, full: bool = False, eps: float = 1e-8, reduction: str = 'mean', mode: str = 'diag',
                 chunk: None) -> None:
    ...
    def forward(self, input: Tensor, target: Tensor, var: Tensor) -> Tensor:
        return multi_gaussian_nll_loss(input, target, var, full=self.full, eps=self.eps, reduction=self.reduction,
                                       mode=self.mode, chunk=self.chunk)
